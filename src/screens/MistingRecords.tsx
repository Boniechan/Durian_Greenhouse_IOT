...
import { MistingRecord } from '../types/mistingFan';
import {
  getLatestMistingActivity,
  formatDuration,
  formatDateString,
} from '../services/mistingService';
...
export default function MistingRecords() {
  const navigation = useNavigation<NavigationProp>();
  const [records, setRecords] = useState<GroupedRecords[]>([]);
  const [refreshing, setRefreshing] = useState(false);
  const [loading, setLoading] = useState(true);

  const fetchMistingRecords = async () => {
    try {
      const allRecords = await getLatestMistingActivity();

      // Group records by date
      const grouped: { [key: string]: MistingRecord[] } = {};
      ...
      console.error('Error fetching misting records:', error);
    } finally {
      setLoading(false);
    }
  };
  ...
  const renderRecord = ({ item }: { item: MistingRecord }) => {
    ...
    return (
      <View style={styles.container}>
        <View style={styles.header}>
          <Text style={styles.title}>💨 Misting Records</Text>
  ...
  return (
    <View style={styles.container}>
      ...
      {records.length === 0 ? (
        <View style={styles.emptyState}>
          <MaterialCommunityIcons name="water-off" size={48} color="#9CA3AF" />
          <Text style={styles.emptyTitle}>No misting activity yet</Text>
          <Text style={styles.emptyText}>
            Misting system activity will be recorded here when it activates and deactivates
          </Text>
        </View>
      ) : (
        <SectionList
