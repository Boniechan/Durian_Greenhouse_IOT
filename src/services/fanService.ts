...
import { FanRecord, FanDayStats } from '../types/mistingFan';
...
function normalizeFanRecord(id: string, data: any): FanRecord {
...
export async function logFanActivation(
  temperature: number,
  humidity: number
  ...
  try {
    ...
    const docRef = await addDoc(collection(firestore, FAN_COLLECTION), record);
    ...
    console.error('Error logging fan activation:', error);
    return null;
  }
}

export async function logFanDeactivation(
  recordId: string,
  ...
  try {
    const recordRef = doc(firestore, FAN_COLLECTION, recordId);
    ...
    console.error('Error logging fan deactivation:', error);
    return false;
  }
}
...
export async function getTodayActivationCount(): Promise<number> {
  try {
    ...
    const q = query(
      collection(firestore, FAN_COLLECTION),
...
export async function getFanRecordsForDate(date: string): Promise<FanRecord[]> {
  try {
    const q = query(
      collection(firestore, FAN_COLLECTION),
    ...
    const querySnapshot = await getDocs(q);
    return querySnapshot.docs.map((doc) => normalizeFanRecord(doc.id, doc.data()));
  } catch (error) {
    console.error('Error fetching fan records for date:', error);
    return [];
  }
}

export async function getFanRecordsForDateRange(
  startDate: string,
  endDate: string
): Promise<FanRecord[]> {
  try {
    ...
    return querySnapshot.docs
      .map((doc) => normalizeFanRecord(doc.id, doc.data()))
    ...
    console.error('Error fetching fan records for date range:', error);
    return [];
  }
}

export async function getFanDailyStats(date: string): Promise<FanDayStats | null> {
  try {
    const records = await getFanRecordsForDate(date);
...
export async function getLatestFanActivity(daysBack: number = 30): Promise<FanRecord[]> {
  try {
    const now = new Date();
    ...
    return await getFanRecordsForDateRange(startDateString, endDateString);
  } catch (error) {
    console.error('Error fetching latest fan activity:', error);
    return [];
  }
}
